...
while cap.isOpened():
    ...
    # --- Count white pixels (i.e., lane pixels in ROI) ---
    white_pixel_count = cv2.countNonZero(masked_lane)

    # --- Draw text with area (pixel count) ---
    lane_highlight = frame.copy()
    cv2.putText(
        lane_highlight,
        f"Lane Area (pixels): {white_pixel_count}",
        (10, 30),
    ...
    for cnt in contours:
        ...
        if 4 <= h <= 20 and aspect_ratio > 1.2 and area > 80:
