...
while cap.isOpened():
    ...
    # Define trapezoidal ROI with updated height (60%)
    top_y = int(0.6 * new_height)  # Increased to 60% of height
    ...
    trapezium_roi = np.array([
        ...
        [right_x, top_y],        # Top-right (60% height)
        [left_x, top_y]          # Top-left (60% height)
    ], np.int32)
        ...
        if result.boxes is not None:  # Ensure detections exist
            for box, cls_id in zip(result.boxes.xyxy, result.boxes.cls):
                ...
                if class_name in allowed_classes:
                    # Check if the bounding box is inside the trapezoidal ROI
